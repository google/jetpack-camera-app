...
object SurfaceTransformationUtil {
    ...
    fun isViewportAspectRatioMatchPreviewView(
        ...
        val rotatedViewportSize: Size = getRotatedViewportSize(transformationInfo, resolution)
    ...
    private fun getPreviewViewViewportRectForMismatchedAspectRatios(
        ...
        val rotatedViewportSize = getRotatedViewportSize(transformationInfo, resolution)
    ...
    fun getSurfaceToPreviewViewMatrix(
        ...
        val previewViewCropRect: RectF =
            if (isViewportAspectRatioMatchPreviewView(
                    transformationInfo,
                    previewViewSize,
                    resolution
                )
                ...
                getPreviewViewViewportRectForMismatchedAspectRatios(
                    transformationInfo, previewViewSize, resolution
                )
            }
        val resolutionRect = RectF(0F, 0F, resolution.width.toFloat(), resolution.height.toFloat())
        val matrix = TransformUtils.getRectToRect(
            resolutionRect, previewViewCropRect, transformationInfo.rotationDegrees
        )
        if (isFrontCamera && transformationInfo.hasCameraTransform()) {
            ...
            if (TransformUtils.is90or270(transformationInfo.rotationDegrees)) {
                ...
                matrix.preScale(
                    1f,
                    -1f,
                    resolutionRect.centerX(),
                    resolutionRect.centerY()
                )
            } else {
